...
public class MsgProcessorThread implements Runnable {
    ...
    public static final int SLEEPTIME_MILASEC = 10;
    private boolean stopRunning = false;
    private MainServer mainChat = null; //Needed???, Yes
    ...
    private void processMsg(MessageIn msg) {
        ...
        if (!u.getMuted()) {
            ...
            if (msg instanceof MsgInAddAdmin) {
                if (u.isAdmin()) {
                    User u2 = getUser(((MsgInAddAdmin) msg).getReciver());
                    System.out.println(u.getName() + " can make admin " + u2.getName());
                    if (u2 != null) {
                        if (u2.isAdmin()) {
                            u2.setAdmin(false);
                        } else {
            ...
            if (msg instanceof MsgInMute) {
                if (u.isAdmin()) {
                    User u2 = getUser(((MsgInMute) msg).getReciver());
                    System.out.println(u.getName() + " can mute " + u2.getName());
                    if (u2 != null && !u2.isAdmin()) {
                        if (u2.getMuted()) {
                            u2.setMuted(false);
                        } else {
                            u2.setMuted(true);
                    ...
                    System.out.println(u.getName() + " is not an admin.");
                }
                return;
            }
            ...
            if (msg instanceof MsgInChannelTextMe) {
                channelTextMe((MsgInChannelTextMe) msg, u);
                return;
            }
            ...
            System.out.println("User was muted.");
            return;
        }
    ...
    private void relationList(MsgInRelationList msg, User u) {
        ...
        if (userRelations == null) {
            ...
            ErrorLog.addMessage("%&%& AlexChatMsgProess Usrtrelation lst not intilized");
            return;
        }
        ...
        while (loopc < size) {
            ...
            if (cur != null) {
                ...
                online[loopc] = userRelations.findFriend(cur.getSecondName());
            }
            loopc++;
        }

        UserMsgBox box = u.getUserMsgBox();
        box.addMessage(new MsgOutRelationList(u.getName(), names, comments, friend, ignore, online));
    ...
    private void subscribedChannels(MsgInSubChannels msg, User u) {
        ...
        while (loopc < size) {
            ...
            if (c != null) {
                try {
                    if (c.getUserNames() != null) {
                        for (int i = 0; i < c.getUserNames().length; i++) {
                            User u2 = getUser(c.getUserNames()[i]);
                            if (u2 != null && u2.isAdmin()) {
                                admins.add(u2.getName());
                            }
                        }
                    }
        ...
        MsgOutSubChannels MOSC = new MsgOutSubChannels(u.getName(), cList, uList);
        MOSC.setAdmin(admins);
        box.addMessage(MOSC);
    ...
    private void channelKick(MsgInChannelKick msg, User u) {
        ...
        //Dose the channel even have this user?
        ...
        //if the user is the channel admin and not a real admin, and the channel is set to not kick
        if ((u == c.getAdmin() || u.isAdmin() == false) && c.getAdminCanKick() == false) {
            String errText = "Could not kick user named <" + msg.getUserToBeKicked() + "> in the channel <" + c.getChannelName() + "> ";
            errText += "Channel Admins can not kick users in this channel.  Full admins can";
            sendErrorMsg(errText, u);
            return;
        }
    ...
    /**
     * Handles a user sending me text to a channel - I know I am a bad person for doing this
     */
    private void channelTextMe(MsgInChannelTextMe msg, User u) {
        //first see if the channel exsits,
        Channel c = channelList.getChannel(msg.getChannelName());
        if (c == null) {
            String errText = "Could not send text to channel <" + msg.getChannelName() + "> ";
        ...
        while (loopc < size) {
            try {
                ...
                if (ur != null && ur.isIgnore() == true) {
                    ...
                    box.addMessage(new MsgOutChannelTextMe(curUser.getName(), msg.getChannelName(),
                            u.getName(), msg.getMessage()));
                }


            } catch (Exception e) {
                String message = "0: WARNING, Alex's chatProgram.ChannelTextMe \n1: THREAD CONTINUING, MSG FAILED ";
                message += "\n2: unexpected error! " + e.getLocalizedMessage();
                System.out.println(message);
            }
            loopc++;
    ...
    private User getUser(String userString) {
        User u = userList.getUser(userString);
        if (u == null) {
            //just make them,
            try {
                sql s = SQLConstants.getSQL();
                u = userList.loadPreExistingUser(userString, s);
                SQLConstants.closeSQL(s);
            } catch (Exception e) {
