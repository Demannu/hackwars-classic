...
public class LoadXML{
	...
	public void setDocument(Document domTree){
	...
	public void loadByteArray(byte Data[]) throws Exception{
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		factory.setCoalescing(true);
		factory.setValidating(false);
		try {
 			DocumentBuilder builder = factory.newDocumentBuilder();
			domTree = builder.parse((InputStream)(new ByteArrayInputStream(Data)));
		} catch (Exception e) {
			e.printStackTrace();
			throw(new Exception("CDoc.loadURL() : Error parsing URL."));
		}
	...
	public void loadString(String xml) throws Exception{
		loadByteArray(xml.getBytes());
		/*DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		factory.setCoalescing(true);
		factory.setValidating(false);
		try{
 			DocumentBuilder builder = factory.newDocumentBuilder();
			domTree = builder.parse(new StringBufferInputStream(xml));
 			root=(Node)domTree;
		}catch(Exception e){
			throw(new Exception("CDoc.loadFile() : Error parsing file."+e));
		}*/
	...
	public Node findNode(Node parent,String name,int skip){
		...
		for(int i=0;i<parent.getChildNodes().getLength();i++){
			...
			if(temp.getNodeName()==name&&cmatch>=skip)
				return(temp);
			else if(temp.getNodeName()==name)
				cmatch++;
		}
	...
	private RMatch findNodeRAN(Node parent,String name,int skip){
		...
		if(parent.getNodeName().equals(name)&&skip<=1){
			return(new RMatch(parent,0));
		}else if(parent.getNodeName()==name)
			skip--;
	...
	// specified. Parent must also be provided.
	///////////////////////////////////////////////////
	public void insertBefore(Node parent,Node n_new,Node n_ipoint){
		Node temp=domTree.importNode(n_new,true);
		parent.insertBefore(temp,n_ipoint);
	}
}//Tested.