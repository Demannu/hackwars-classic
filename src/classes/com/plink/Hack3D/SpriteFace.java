...
public class SpriteFace{
	...
	private HashMap ControlDisplayData=new HashMap();//The data used to label and display specific controls.
	...
	public void load(String modelPath){
		...
		//	BufferedReader BR=new BufferedReader(new FileReader(F));
			...
			while((data=BR.readLine())!=null){
				...
				if(data.contains("#GROUPID	Name	XROT(name,min,max)")){
					currentParse=CONTROLS;
					continue;
				}
				...
				if(currentParse==CONTROLS){
					SliderInformation SI=new SliderInformation();
					SI.parse(ControlDisplayData,data);
				}
		...
		System.out.println(ControlDisplayData);
	...
	*/
	...
	/**
	Get the slider information for a specific group.
	*/
	public SliderInformation getSliderInformation(String id){
		return((SliderInformation)ControlDisplayData.get(id));
	}
